...
**See what each appliance is actually costing you.** An appliance electricity cost calculator that shows what every appliance costs to run — per month and per year — on *your* electricity rate, then ranks them so the biggest money-drainers come first. It walks you through three quick steps — your rate, your appliances, the result — and lets you share that result as a link. 100% client-side, zero dependencies, works fully offline.
...
- **Guided in three quick steps** — set your rate, tick the appliances you have and how many of each, then see what they cost. No wall of controls to face up front.
...
- **Shareable result** — one tap builds a link that encodes your rate and appliances, so anyone who opens it sees the same breakdown (and on phones the native share sheet is used). Nothing is uploaded — the whole setup rides inside the link itself.
...
Your appliance list, currency, and rate are saved in your browser's local storage, so they pick up where you left off between visits. "Share result" copies a link (or opens the native share sheet) that reproduces your exact breakdown for someone else; "Start over" clears your picks.
